 ...
 * java GeneralizedSpread <solutionFrontFile> <trueFrontFile>
 * <numberOfObjectives> It is recommendable to see the metric description.
 * Reference: A. Zhou, Y. Jin, Q. Zhang, B. Sendhoff, and E. Tsang Combining
 * model-based and genetics-based offspring generation for multi-objective
 * optimization using a convergence criterion, 2006 IEEE Congress on
 * Evolutionary Computation, 2006, pp. 3234-3241.
 */
public class GeneralizedSpread
{
	// MetricsUtil provides some utilities for implementing the metric
	...
	/**
	 * Calculates the generalized spread metric. Given the pareto front, the
	 * true pareto front as <code>double []</code> and the number of objectives,
	 * the method return the value for the metric.
	 * 
	...
	public double generalizedSpread(double[][] paretoFront, double[][] paretoTrueFront, int numberOfObjectives)
	{
		...
		Comparator<double[]> localComparator = new LexicoGraphicalComparator();
		Arrays.sort(normalizedFront, localComparator);
		...
		else
		{
			...
			for (int i = 0; i < normalizedFront.length; i++)
			{
				mean += Math.abs(utils_.distanceToNearestPoint(normalizedFront[i], normalizedFront) - dmean);
			}
