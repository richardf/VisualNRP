 ...
 * used also as a command line by typing: "java InvertedGenerationalDistance
 * <solutionFrontFile> <trueFrontFile> <numberOfObjectives>" Reference: Van
 * Veldhuizen, D.A., Lamont, G.B.: Multiobjective Evolutionary Algorithm
 * Research: A History and Analysis. Technical Report TR-98-03, Dept. Elec.
 * Comput. Eng., Air Force Inst. Technol. (1998)
 */
public class InvertedGenerationalDistance
{
	MetricsUtil utils_; // utils_ is used to access
	// to the
	...
	public double invertedGenerationalDistance(double[][] front, double[][] trueParetoFront, int numberOfObjectives)
	{
		...
		for (int i = 0; i < normalizedParetoFront.length; i++)
			sum += Math.pow(utils_.distanceToClosedPoint(normalizedParetoFront[i], normalizedFront), pow_);
